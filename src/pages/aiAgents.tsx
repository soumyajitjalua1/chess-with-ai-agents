...
type AIAgent = 'basic' | 'intermediate' | 'advanced';
type PlayerColor = 'white' | 'black';

const AIChessChallenge: React.FC = () => {
  // Game state
  ...
  const [selectedAgent, setSelectedAgent] = useState<AIAgent>('intermediate');
  ...
  const makeAIMove = (chess: Chess) => {
    ...
    switch(selectedAgent) {
      case 'basic':
        // Basic Agent: Random moves
        selectedMove = moves[Math.floor(Math.random() * moves.length)];
        break;
        
      case 'intermediate':
        // Intermediate Agent: Prioritizes captures and checks
      ...
      case 'advanced':
        // Advanced Agent: Uses piece value evaluation
        const valuePieces = { p: 1, n: 3, b: 3, r: 5, q: 9, k: 0 };
        ...
        selectedMove = Math.random() < 0.8 ? sortedMoves[0] : moves[Math.floor(Math.random() * moves.length)];
        break;
    }
      ...
      <main className="container mx-auto px-4 py-8">
        {gameState === 'setup' ? (
          ...
          >
            ...
            <motion.p className="text-gray-400 mb-8 text-center" variants={itemVariants}>
              Test your skills against our specialized AI agents
            </motion.p>
            
            <motion.div className="bg-chess-darker p-6 rounded-lg mb-6" variants={itemVariants}>
              <h3 className="text-xl font-semibold mb-4">Challenge Settings</h3>
              
              <div className="mb-6">
                <label className="block text-gray-400 mb-2">Select AI Agent</label>
                <div className="grid grid-cols-1 gap-2">
                  <button
                    className={`p-4 rounded text-left ${selectedAgent === 'basic' ? 'bg-red-500 text-white' : 'bg-gray-800 hover:bg-gray-700'}`}
                    onClick={() => setSelectedAgent('basic')}
                  >
                    <div className="font-semibold">Basic Bot</div>
                    <div className="text-sm text-gray-300">Random moves, perfect for beginners</div>
                  </button>
                  <button
                    className={`p-4 rounded text-left ${selectedAgent === 'intermediate' ? 'bg-red-500 text-white' : 'bg-gray-800 hover:bg-gray-700'}`}
                    onClick={() => setSelectedAgent('intermediate')}
                  >
                    <div className="font-semibold">Tactical Bot</div>
                    <div className="text-sm text-gray-300">Prioritizes captures and checks</div>
                  </button>
                  <button
                    className={`p-4 rounded text-left ${selectedAgent === 'advanced' ? 'bg-red-500 text-white' : 'bg-gray-800 hover:bg-gray-700'}`}
                    onClick={() => setSelectedAgent('advanced')}
                  >
                    <div className="font-semibold">Advanced Bot</div>
                    <div className="text-sm text-gray-300">Uses piece value evaluation</div>
                  </button>
                </div>
              </div>
              <div className="mb-6">
              ...
              <div className="flex-1">
                <div className="bg-chess-darker p-4 rounded-lg shadow-lg">
                  ...
                  <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center">
                      ...
                      <div className="ml-2">
                        <div className="font-semibold">NeoMind AI</div>
                        <div className="text-xs text-gray-400">
                          {selectedAgent.charAt(0).toUpperCase() + selectedAgent.slice(1)}
                        </div>
                      </div>
                    </div>
                    <div className={`px-4 py-2 rounded bg-gray-700 font-mono ${aiTime < 30 ? 'text-red-500' : ''}`}>
                      {formatTime(aiTime)}
                    </div>
                  </div>
                ...
                <div className="relative w-[400px] mx-auto mb-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className={`px-4 py-2 rounded bg-gray-700 font-mono ${playerTime < 30 ? 'text-red-500' : ''}`}>
  ...
  export default AIChessChallenge;
